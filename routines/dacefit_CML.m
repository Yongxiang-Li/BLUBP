function  [dmodel, perf] = dacefit_CML(design, regr, corr, theta0, lob, upb, options)
    nInput = 4;
    if nargin < nInput
...
function  [obj, fit] = objfunc(theta, par)
    ...
    n = sum([par.design.n]);    w = n;     
    FKF = 0;    FKY = 0;    YKY = 0;    detK = 0;
    if par.UseParallel
        parfor i = 1 : k
            C = chol(feval(corr,theta,design(i).S))';
            IFi = C \ design(i).F;
            IYi = C \ design(i).Y;
            FKF = FKF + IFi'*IFi/w;
            FKY = FKY + IFi'*IYi/w;
            YKY = YKY + IYi'*IYi/w;
            detK = detK + 2*sum(log(full(diag(C))))/w;
        end
    else
        for i = 1 : k
            C = chol(feval(corr,theta,design(i).S))';
